...
static struct ip_route *ip_route_add(ip_addr_t network, ip_addr_t netmask,
  ...
  if (nexthop != IP_ADDR_ANY) {
    ...
    infof("%s/%s dev %s src %s", ip_addr_ntop(network, addr1, sizeof(addr1)),
          ip_addr_ntop(netmask, addr2, sizeof(addr2)),
          NET_IFACE(iface)->dev->name,
          ip_addr_ntop(iface->unicast, addr4, sizeof(addr4)));
  }
...
struct ip_iface *ip_iface_alloc(const char *unicast, const char *netmask) {
  ...
  /**
   * network address = unicast address & netmask
   * wildcard mask with host bits set = ~netmask
...
static void ip_print(const uint8_t *data, size_t len) {
  ...
  hl = hdr->vhl & 0x0f;
  hlen = hl * 4;
  fprintf(stderr, "        vhl: 0x%02x [v: %u, hl: %u, hlen: %u]\n", hdr->vhl,
          v, hl, hlen);
  ...
  fprintf(stderr, "        offset: 0x%04x [flags=0x%02x, offset=0x%02x]\n",
...
int ip_protocol_register(uint8_t protocol, ip_protocol_handler_t handler) {
  ...
  for (entry = protocols; entry; entry = entry->next) {
    if (entry->protocol == protocol) {
      errorf("protocol %u already registered", protocol);
      return -1;
    }
  }
...
static void ip_input(const uint8_t *data, size_t len, struct net_device *dev) {
  ...
  if (offset & IP_HDR_FLAG_MF || offset & IP_HDR_OFFSET_MASK) {
    errorf("fragmented packet not supported");
    return;
  }
  // get ip interface connected to the network device
...
ssize_t ip_output(uint8_t protocol, const uint8_t *data, size_t len,
  ...
  if (src == IP_ADDR_ANY && dst == IP_ADDR_BROADCAST) {
    errorf("src address is required for broadcast");
    return -1;
  }
  route = ip_route_lookup(dst);
  if (!route) {
  ...
  if (NET_IFACE(iface)->dev->mtu < IP_HDR_SIZE_MIN + len) {
    errorf("too long, dev=%s, mtu=%u, len=%zu", NET_IFACE(iface)->dev->name,
           NET_IFACE(iface)->dev->mtu, len);
    return -1;
  }
  id = random16();
  plen =
      ip_build_packet(protocol, data, len, id, 0, src, dst, buf, sizeof(buf));
  if (plen == -1) {
    errorf("ip_build_packet() failure");
    return -1;
  }
  if (ip_output_device(iface, buf, plen,
                       route->nexthop ? route->nexthop : dst) == -1) {
