...
static int app_main(void) {
  ...
  debugf("connection accepted from %s, desc=%d",
         ip_endp_ntop(remote, ep, sizeof(ep)), new_desc);
  debugf("press Ctrl+C to terminate");
  while (!terminate) {
    n = tcp_cmd_receive(new_desc, buf, sizeof(buf));
    if (n <= 0) {
      warnf("connection closed or error occurred");
      break;
    }
    debugf("%zd bytes received", n);
    hexdump(stderr, buf, n);
    tcp_cmd_send(new_desc, buf, n);
  }
  tcp_cmd_close(new_desc);
  tcp_cmd_close(desc);
  debugf("terminated echo server");
  return 0;
}
